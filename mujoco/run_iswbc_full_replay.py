...
def arg_parse():
    ...
    parser.add_argument('--experiment', type=str, default='full_replay')

    parser.add_argument('--dataset-dir', type=str, default="dataset/rlkit")
    parser.add_argument('--dataset-name', type=str, default="full_replay-v2")
    parser.add_argument('--num-expert-trajectory', type=int, default=1)
    parser.add_argument('--stochastic-policy', type=int, default=1, choices=[0, 1])
...
def main(args=arg_parse()):
    ...
    imperfect_dataset = dataset_utils.get_dataset(args.dataset_dir, args.env_id, args.dataset_name)
    imperfect_states, imperfect_actions = imperfect_dataset["observations"], imperfect_dataset["actions"]
    imperfect_states, imperfect_actions = imperfect_states[:int(1e6)], imperfect_actions[:int(1e6)]
